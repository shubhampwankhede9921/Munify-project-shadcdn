...
export async function getUserAccount(token: string, username: string): Promise<any> {
  // Import axios instance directly to set header explicitly with JWT token
  const { api } = await import('./api')
  const response = await api.get(`/users/perdix/${username}`, {
    headers: {
